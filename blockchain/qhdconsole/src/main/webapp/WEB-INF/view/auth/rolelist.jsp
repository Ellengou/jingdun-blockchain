		...
		<div class="container-fluid">
			<div id="pad-wrapper">
				<div class="table-products">
					<form id="SearchForm" name="SearchForm" action="" class="" method="get" action="${contextPath}/auth/rolelist.htm">
						<div class="row-fluid header">
							<h4>
								<legend>
									角色管理
									<small>管理平台的角色管理</small>
								</legend>
							</h4>
							<div class="pull-right">
								<input id="sname" name="sname" type="text" class="search" placeholder="Type a role name..." value="${param.sname}" />
								<a class="btn-flat new-product" onclick="doSearch()">搜索角色</a>
								&nbsp;
								<a class="btn-flat success pull-right" href="${contextPath}/auth/roleform.htm">
									<span>&#43;</span> 新增角色
								</a>
							</div>
						</div>
								...
								<tbody>
									<c:forEach items="${page.result}" varStatus="status" var="v">
										<tr>
											...
											<td>${v.username}</td>
											<td>${v.createdate}</td>
											<td class="description">${v.remarks}</td>
											<td class="align-right">
												<a href="${contextPath}/auth/roleform.htm?rid=${v.rid}" class="btn-flat default">修改</a>
												<c:if test="${v.sname != '超级管理员'}">
													<a onclick="doDel(${v.rid},${status.index});" class="btn-flat danger">删除</a>
												</c:if>
											</td>
										</tr>
									</c:forEach>
...
<script type="text/javascript">
	...
	function doDel(id, index) {
		if (confirm("确认删除？")) {
			ajax_get("${contextPath}/auth/roledelete.htm", "rid=" + id, function(data) {
