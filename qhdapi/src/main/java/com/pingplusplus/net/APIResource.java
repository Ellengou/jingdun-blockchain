...
public abstract class APIResource extends PingppObject {
	...
	public static final Gson GSON = new GsonBuilder().setFieldNamingPolicy(FieldNamingPolicy.LOWER_CASE_WITH_UNDERSCORES)
			.registerTypeAdapter(Charge.class, new ChargeDeserializer()).registerTypeAdapter(RedEnvelope.class, new RedEnvelopeDeserializer())
	...
	static Map<String, String> getHeaders(String apiKey) {
		...
		headers.put("User-Agent", String.format("Pingpp/v1 JavaBindings/%s", Pingpp.VERSION));
	 ...
	 * @throws IOException
	 */
	private static java.net.HttpURLConnection createPingppConnection(String url, String apiKey) throws IOException {
		URL pingppURL = null;
	 ...
	 * @throws APIConnectionException
	 */
	private static java.net.HttpURLConnection createGetConnection(String url, String query, String apiKey) throws IOException, APIConnectionException {
		String getURL = formatURL(url, query);
		java.net.HttpURLConnection conn = createPingppConnection(getURL, apiKey);
		conn.setRequestMethod("GET");

		return conn;
	}

	private static java.net.HttpURLConnection createDeleteConnection(String url, String query, String apiKey) throws IOException,
	 ...
	 * @throws APIConnectionException
	 */
	private static java.net.HttpURLConnection createPostConnection(String url, String query, String apiKey) throws IOException,
			APIConnectionException {
		java.net.HttpURLConnection conn = createPingppConnection(url, apiKey);
		...
		if (url.indexOf("batch_transfers") != -1) {
			//			System.out.println("===as: " + (query + "/v1/batch_transfers" + Long.toString(nowtime)));
			signature = generateSign(query + "/v1/batch_transfers" + Long.toString(nowtime));
			conn.setRequestProperty("Pingplusplus-Request-Timestamp", nowtime + "");
			//		} else if (url.indexOf("charges") != -1) {
			//			System.out.println("===as: " + (query + "/v1/charges" + Long.toString(nowtime)));
			//			signature = generateSign(query + "/v1/charges" + Long.toString(nowtime));
		} else {
			signature = generateSign(query);
		}
	 ...
	 * @throws APIConnectionException
	 */
	private static java.net.HttpURLConnection createPutConnection(String url, String query, String apiKey) throws IOException, APIConnectionException {
		java.net.HttpURLConnection conn = createPingppConnection(url, apiKey);
		...
		for (Map.Entry<String, Object> entry : params.entrySet()) {
			...
			if (value instanceof Map<?, ?>) {
				...
				throw new InvalidRequestException("You cannot set '" + key + "' to an empty string. "
						+ "We interpret empty strings as null in requests. " + "You may set '" + key + "' to null to delete the property.", key, null);
			} else if (value == null) {
				flatParams.put(key, "");
			} else {
	 ...
	 * @throws APIConnectionException
	 */
	private static PingppResponse makeURLConnectionRequest(APIResource.RequestMethod method, String url, String query, String apiKey)
			throws APIConnectionException {
		...
		try {
			...
			throw new APIConnectionException(String.format("IOException during API request to Pingpp (%s): %s "
					+ "Please check your internet connection and try again. If this problem persists,"
					+ "you should check Pingpp's service status at https://pingxx.com/status.", Pingpp.getApiBase(), e.getMessage()), e);
		} finally {
			if (conn != null) {
				conn.disconnect();
			}
	 ...
	 * @throws APIException
	 */
	protected static <T> T request(APIResource.RequestMethod method, String url, Map<String, Object> params, Class<T> clazz)
			throws AuthenticationException, InvalidRequestException, APIConnectionException, APIException, ChannelException, RateLimitException {
		if ((Pingpp.apiKey == null || Pingpp.apiKey.length() == 0)) {
			throw new AuthenticationException("No API key provided. (HINT: set your API key using 'Pingpp.apiKey = <API-KEY>'. "
					+ "You can generate API keys from the Pingpp web interface. " + "See https://pingxx.com for details.");
		}
	...
	private static void handleAPIError(String rBody, int rCode) throws InvalidRequestException, AuthenticationException, APIException,
			ChannelException, RateLimitException {
	...
	private static String generateSign(String data) throws IOException {
		...
		String trimmedPrivateKey = Pingpp.privateKey.replaceAll("(-+BEGIN (RSA )?PRIVATE KEY-+\\r?\\n|-+END (RSA )?PRIVATE KEY-+\\r?\\n?)", "");
