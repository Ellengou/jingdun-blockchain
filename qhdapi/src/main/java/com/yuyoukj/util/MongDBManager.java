...
public class MongDBManager {
	final static Log logger = LogFactory.getLog(MongDBManager.class);
	...
	public static void setMgdbs(Map<String, MongDBUtil> mgdbs) {
		MongDBManager.mgdbs = mgdbs;
	}

	private String osmads;
	private DBCollection liveDbCollection;

	public MongDBManager(String ip, Integer port, String osmads) {
		try {
			this.osmads = osmads;
			...
			liveDbCollection = mg.getDB("osmlive").getCollection("osm_live_data");
	...
	public DBCollection getLivefeedback(String dbkey) throws Exception {
		MongDBUtil mgdb = getMgdb(dbkey);
		DBCollection livefeedback = null;
		if (null != mgdb) {
			livefeedback = mgdb.getLivefeedback();
		}
		return livefeedback;
	}
	...
	public DBCollection getSDKLogfeedback(String dbkey) throws Exception {
		MongDBUtil mgdb = getMgdb(dbkey);
		DBCollection SDKlogfeedback = null;
		if (null != mgdb) {
			SDKlogfeedback = mgdb.getSDKLogfeedback();
		}
